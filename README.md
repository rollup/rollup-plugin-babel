...
export default {
	...
	output: [
		{ file: 'bundle.js', format: 'esm' },
		{
			file: 'bundle.es5.js',
			format: 'esm',
			plugins: [babel({ transformGenerated: true, presets: ['@babel/env'] })],
		},
	],
};
```
...
As with the `transformGenerated` option this plugin will run _after_ Rollup has done all its transformations, it needs to make sure it preserves the semantics of Rollup's output format. This is especially important for Babel plugins that add, modify or remove imports or exports.

If you are having issues and you are using `@babel/env`, it is recommended to add the modules option with a value that matches the format generated by Rollup, e.g.:

```
presets: [['@babel/env', { modules: 'cjs' }], ...]
```

One important case is when using runtime helpers, see below.
