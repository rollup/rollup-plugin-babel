...
describe('rollup-plugin-babel', function() {
	...
	it('supports customizing the loader', () => {
		return rollup
			.rollup({
				input: 'samples/class/main.js',
				plugins: [
					babelPlugin.custom(babel => {
						function myPlugin() {
							return {
								visitor: {},
							};
								...
								return Object.assign(
									{
										...
										// TODO find something we can test against
										// custom: { opt1, opt2 },
									},
			...
			.then(({ code }) => {
				assert.ok(code.indexOf('// Generated by some custom loader') !== -1, code);
				assert.ok(code.indexOf('function _classCallCheck') !== -1, code);
			});
	});
});
